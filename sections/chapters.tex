...
\begin{itemize}
    ...
    \item $2^E$ joukon $E$ osajoukkojen kokoelmaa
\end{itemize}
\hrule
...
Tässä tutkielmassa rajoitettaan tarkasteltavien polkujen joukon 1-Lipschitz-kuvauksiin, jotka kuvautuvat johonkin konveksiin $\R^n$ osajoukkoon $X$.\whytho{miksi?} Merkitään kaikkien tällaisten 1-Lipschitz kuvauksien $\gamma: \Rp \to X$  joukkoa $K$. 
...
\begin{definition}\label{def:liikennesuunnitelma}
    Mitta $\P: K \to \R^+$ joukossa $K$ on \textbf{liikennesuunnitelma}, jos
    \begin{equation*}
     \int_K T(\gamma) \, \d \P (\gamma) < \infty.   
    \end{equation*}
    Merkitään lisäksi joukon $X$ liikennesuunnitelmien kokoelmaa $TP(X)$.  \\ \whytho{Onko tässä X vain joukko vai aiemmin mainittu kompakti Rn osajoukko?}
\end{definition}

Liikennesuunnitelman määritelmä sallii, että $\P$-nollamittainen
joukko polkuja $\gamma$ on pysähtymisajaltaan äärettömiä, eli joille $T(\gamma) = \infty$. Otetaan myös käyttöön jatkon kannalta määritelmä 
...
\end{theorem}
\begin{proof}
\cite[p. 27]{OptimalTransportationNetworks}(joss.-versio)
\end{proof}
...
\end{align*}
\whytho{Näytettävä, että $K$ ja $\Omega$ Borelin avaruuksia. Mistä saadaan sigma-algebrat?}
\end{proof}



\subsection{Liikennesuunnitelmien {stabiliteetti}}
Tässä kappaleessa perehdytään liikennesuunnitelmien heikkoon suppenemiseen, ja pysähtymisajan sekä pituuden puolijatkuvuuteen. Sovitaan ensin merkintä jatkuville funktioille, joiden kantaja on kompakti.
\begin{definition}
